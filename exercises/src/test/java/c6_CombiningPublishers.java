...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        Flux<Void> tasks = taskExecutor().flatMap(x -> x);
    ...
    public void streaming_service() {
        Flux<Message> messageFlux = streamingService().flatMapMany(x -> x);
    ...
    public void i_am_rubber_you_are_glue() {
        ...
        Flux<Integer> numbers2 = Flux.concat(numberService1(), numberService2());
    ...
    public void task_executor_again() {
        Flux<Void> tasks = taskExecutor().concatMap(x -> x);
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) -> {
            if (Objects.equals(Objects.requireNonNull(signal.get()).metaData, "spam")) {
    ...
    public void instant_search() {
        Flux<String> suggestions =
                userSearchInput().switchMap(this::autoComplete);
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds =
                tasksToExecute()
                        .concatMap(x -> x.flatMap(y -> commitTask(y).thenReturn(y)));
    ...
    public Mono<String> chooseSource() {
        return sourceA()
                .flatMap(x -> sourceRef.get().equals("A") ?
                        Mono.just(x) :
                        sourceRef.get().equals("B") ?
                                sourceB() : Mono.empty());
    }
